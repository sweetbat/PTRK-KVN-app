...
class OlcboxVpnService : VpnService() {
    ...
    private suspend fun startMihomoFullTunnel(
        ...
        // Tear down the other engine first (olcRTC + hev).
        ...
        val mode = runCatching {
            applicationContext.vpnPrefDataStore.data.first()[KEY_MIHOMO_MODE]
        ...
        try {
            ...
            addLog("Mihomo setup mode=$mode proxy=$proxyName path=$yamlPath")
            ...
            val pfd = establishMihomoVpnTunnel()
            if (pfd == null) {
                stopMihomoTun()
                return
            }
            // Hand fd ownership to libclash/sing-tun (same as FlClashX).
            val nativeFd = pfd.detachFd()
            runCatching { pfd.close() }
            val started = MihomoEngine.startTun(
                fd = nativeFd,
                protect = { fd -> protect(fd) },
            )
            if (!started) {
                addLog("Mihomo startTun failed")
                stopMihomoTun()
                setStatus(VpnStatus.Error("Mihomo TUN failed"))
                updateNotification("Tunnel failed")
                return
            }
            mihomoTunActive = true
            // Keep a placeholder so revoke/cleanup knows a VPN iface was requested.
            vpnInterface = null
            ...
            resetRecoveryState()
            updateNotification(connectedNotificationText())
            addLog("Mihomo VPN tunnel established")
            startWatchdog()
    ...
    private fun establishMihomoVpnTunnel(): ParcelFileDescriptor? {
        return try {
            ...
            val dns = runCatching {
                JSONObject(optionsJson).optString("dnsServerAddress").ifBlank { null }
            }.getOrNull() ?: "172.19.0.2"
            val builder = Builder()
                .setSession("PTRK-KVN")
                .setMtu(TUN_MTU)
    ...
    private fun startWatchdog() {
        ...
        watchdogJob = scope.launch {
            while (isActive && OlcboxVpnState.status.value is VpnStatus.Connected) {
                ...
                if (mode == AndroidConnectionMode.Tun && !mihomoTunActive && isTunTrafficStalled()) {
